...
function greeter(firstName, state){
	;
}
let me = {
	firstName: "Esteban", 
...
function bigOrSmall(arr){
	let answers = arr.map(function(item){
		if(item > 100) return "big";
		else if(item <= 100) return "small";
	});
	return answers;
}
...
let doubled = [];
doubled = myNumbers.map(function(value){
	return (value * 2);
});
...
let filtered = [];
filtered = myNumbers.filter(value => value > 100);
...
let total = [];
total = myNumbers.reduce(function(acc, curr){
	return acc + curr;
});
...
myNumbers.forEach(function(value){
	myNumbersIndex.push(myNumbers.indexOf(value));
});
...
forTheLoveOfGeorge = notGeorge.map(item =>{
	return "George";
});
...
let totallyAwesome = [];
totallyAwesome = people.reduce(function(acc, curr){
